...
Status epiku: **Faza 1 WYKONANA** (ten plik jest wyłącznie planem — bez zmian w `src/`, `docs/` ani Compose, dopóki użytkownik o to nie poprosi).  
Źródło ustaleń: przegląd architektury `src/cache`, Compose, readiness; decyzje produktowe z 2026-08-22.

Kolejność pracy (gdy ruszy implementacja): **dokumentacja + kontrakty → `src/` → Compose/skrypty → testy**.
...
| Partycja | TAG `modelAlias` + `clientId`; ten sam `clientId` wchodzi do klucza exact |
| Skip | tooling; brak `gatewayKey`; `clientId === 'unknown'`; brak ostatniego usera (tylko semantic); stream v1 |
| Fail-open | Redis / embedding down → chat idzie dalej; `/ready` może być `degraded`, nie `not_ready` |
| Flagi | kod: `SEMANTIC_CACHE_ENABLED=false`; `.env` / Compose tego projektu: `true` |
| Próg | cosine **similarity** default **0.90** (env nadpisuje); w Redis COSINE dystans ≈ `1 − similarity` → cutoff **0.10** |
...
| Testy wektorowe | nie na `test/integration/docker-compose.redis.yml` (`redis:7-alpine`) — osobny stack ze Stackiem; CI bez żywej Ollamy |
...
function isRedisInfrastructureRequired(obj: EnvironmentVariables): boolean {
  const redisKv =
    ...
    (obj.CACHE_BACKEND ?? 'noop').toLowerCase() === 'redis';
  return redisKv || obj.RATE_LIMIT_SMART_ENABLED === true || obj.SEMANTIC_CACHE_ENABLED === true;
}
```
...
```ts
  ...
  const redisConfig: RedisRuntimeConfig = isRedisRequired({
    ...
    ? { host: env.REDIS_HOST ?? 'localhost', port: asPort(env.REDIS_PORT ?? 6379), /* … */ }
    : { host: 'localhost', port: asPort(6379), password: '', db: 0, keyPrefix: 'aigw:' };
...
export function getRedisConsumers(input: RedisRequirementSnapshot): RedisConsumer[] {
...
export function isRedisRequiredFromEnv(env: NodeJS.ProcessEnv = process.env): boolean {
  const cacheEnabled = env.CACHE_ENABLED === 'true';
  return isRedisRequired({
    cache: { enabled: cacheEnabled, backend: parseCacheBackend(env.CACHE_BACKEND, cacheEnabled) },
...
```ts
    ...
    it('should include semantic-cache consumer', () => {
      expect(
        getRedisConsumers({ semanticCacheEnabled: true }),
      ).toEqual(['semantic-cache']);
    });
```
...
export class RedisVectorStoreAdapter implements VectorStore, OnModuleInit {
  ...
  async knn(input): Promise<VectorSearchHit[]> { /* FT.SEARCH @modelAlias:{x} @clientId:{y} => [KNN k @vector $blob]; score = 1 - COSINE distance */ }
  ...
  async upsert(input): Promise<void> { /* HSET entryKey(...) + EXPIRE ttl */ }
}
```
...
export class SemanticCacheService {
  ...
  async lookup(
    ...
    if (this.circuit.isCircuitOpen()) {
      this.appMetrics.recordSemanticCacheLookup(asModelAlias(request.modelAlias), 'error');
      return null;
    }
    try {
      ...
      if (!best || best.similarity < cfg.minSimilarity) {
        this.appMetrics.recordSemanticCacheLookup(asModelAlias(request.modelAlias), 'below-threshold');
        return null;
      }
      this.appMetrics.recordSemanticCacheLookup(asModelAlias(request.modelAlias), 'hit');
      return best.reply;
    } catch {
...
1. Docs EN+PL i README zgodne z tym planem.
2. OpenAPI: opcjonalne `checks.embeddings`.
3. JSON chat: exact → semantic → provider; partycja `modelAlias`+`clientId`; próg 0.90; fail-open.
4. Stream bez cache.
5. Compose baza: gateway + Stack + embedding CPU; 16 GB w docs.
6. Unit + (gdy Faza 4) integration bez Ollamy w CI.

---

## Poza zakresem v1

Replay SSE z cache; semantic lookup na streamie; wspólna Ollama czat+embedding; nowy plik Compose dla Redis; TEI/OpenAI embeddings (port ma to umożliwić później); microservices; zmiana `maxmemory-policy` na LRU.
