...
**Aktualizacja planu:** 
- **2026-06-15** — krok C8.9 wykonany: fasada OpenAI — `reasoning_effort` w DTO + mapper (kompatybilność API, bez działającego backendu /v1/responses)
- **2026-06-14** — krok C8.8 wykonany: chat service — propagacja thinkingContent z providera do odpowiedzi gateway (completeOnce + streamOnce)
- **2026-06-14** — krok C8.5c wykonany: CLI — wsparcie thinking mode w konfiguracji projektu (helpers detection, template generation, model manager)
- **2026-06-14** — krok C8.5b wykonany: walidacja thinking vs maxOutputTokens (cross-validation w `resolve-provider-call-options.ts`)
- **2026-06-14** — krok C8.5c dodany: CLI — wsparcie thinking mode w konfiguracji projektu (helpers detection, template generation, model manager)
...
| **C8. Extended Thinking** | Wsparcie thinking mode dla OpenAI (reasoning_effort) i Anthropic (extended_thinking) | 3-5h | Po C1, C4 | 🔄 **W trakcie** (C8.1–C8.5c, C8.7–C8.10 ✅; C8.11+ planowane) |
...
### Krok C8.5c: CLI — wsparcie thinking mode w konfiguracji projektu

**Status:** ✅ **WYKONANY 2026-06-14**

**Pliki:**
- `src/cli/constants/thinking-capable-models.ts` (nowy)
- `src/cli/templates/gateway-config.template.ts`
- `src/cli/services/model-manager.service.ts`

**Cel:** Zapewnić, że CLI generuje konfigurację zgodną z wymaganiami thinking mode:
1. Dodaje `thinkingEnabled`, `thinkingBudget` do `allowOverrides` (wszystkie modele)
2. Wykrywa thinking-capable modele i ustawia wyższe `maxOutputTokens` defaults
3. Opcjonalnie: dodaje `capabilities.thinking: true` w YAML dla wykrytych modeli

**Akcja:**

#### 1. Helper do wykrywania thinking-capable modelów

**Plik:** `src/cli/constants/thinking-capable-models.ts` (nowy)

```typescript
import type { GatewayProviderType } from 'src/config/provider-types';

/**
 * Lista prefiksów modelId dla modeli wspierających thinking mode (extended reasoning).
 * 
 * Anthropic: claude-opus-4-* (Claude Opus 4.x), claude-sonnet-4-5+
 * Google: gemini-3.* (Gemini 3.0+)
 * OpenAI: POZA ZAKRESEM (wymaga /v1/responses API)
 */
export const THINKING_CAPABLE_MODEL_PATTERNS: Partial<
  Record<GatewayProviderType, RegExp[]>
> = {
  anthropic: [
    /^claude-opus-4-/i,           // Claude Opus 4.x (wszystkie wersje)
    /^claude-sonnet-4-[5-9]/i,    // Claude Sonnet 4.5+
    /^claude-sonnet-[5-9]-/i,     // Claude Sonnet 5.x+
  ],
  google: [
    /^gemini-3\./i,               // Gemini 3.0+
    /^gemini-[4-9]\./i,           // Gemini 4.0+ (przyszłość)
  ],
};

/**
 * Sprawdza, czy dany modelId wspiera thinking mode na podstawie provider type.
 * 
 * @param modelId - ID modelu z gateway.config.yaml (np. 'claude-opus-4-8', 'gemini-3.0-flash')
 * @param providerType - Typ providera ('anthropic', 'google')
 * @returns true gdy model wspiera thinking mode
 */
...
/**
 * Zwraca zalecany default maxOutputTokens w zależności od wsparcia thinking mode.
 * 
 * @param modelId - ID modelu
 * @param providerType - Typ providera
 * @returns Zalecany default maxOutputTokens (8192 dla thinking, 1024 dla non-thinking)
 */
...
**Uzasadnienie regex patterns:**
- **Anthropic:** Claude Opus 4.x wszystkie wersje + Sonnet 4.5+ (thinking wprowadzony w 4.5)
- **Google:** Gemini 3.0+ (ThinkingConfig od wersji 3.0; 2.5 i wcześniejsze NIE wspierają)
- **OpenAI:** Poza zakresem (wymaga implementacji `/v1/responses` API — krok C8.6)

#### 2. Template konfiguracji — allowOverrides + thinking defaults

**Plik:** `src/cli/templates/gateway-config.template.ts`
...
export function generateGatewayConfigTemplate(
  ...
  // 🆕 C8.5c: Provider type lookup dla thinking detection
  const providerTypeMap = new Map(
    input.providers.map((p) => [p.id, p.type]),
  );

  const models = Object.fromEntries(
    input.models.map((model) => {
      // 🆕 C8.5c: Wykryj thinking-capable model
      ...
      return [
        model.alias,
        {
          ...
          capabilities: {
            streaming: true,
            // 🆕 C8.5c: Automatycznie ustaw thinking dla wspieranych modeli
          ...
          policy: {
            ...
            params: {
              defaults: {
                temperature: 0.7,
                // 🆕 C8.5c: Wyższy maxOutputTokens dla thinking-capable models
                maxOutputTokens: recommendedMaxTokens,
                // 🆕 C8.5c: thinkingEnabled domyślnie false (opt-in, wysokie koszty)
                ...(supportsThinking && { thinkingEnabled: false }),
              },
              // 🆕 C8.5c: allowOverrides zawsze zawiera thinking params (nawet dla non-thinking models — forward compatibility)
              allowOverrides: [
                'temperature',
                'maxOutputTokens',
                'topP',
              ...
              bounds: {
                temperature: { min: 0, max: 2 },
                // 🆕 C8.5c: Bounds maxOutputTokens — wyższy sufit dla thinking models
                maxOutputTokens: {
                  min: 1,
...
#### 3. Model manager — defaultModelPolicy z thinking support

**Plik:** `src/cli/services/model-manager.service.ts`
...
// 🆕 C8.5c: Parametry dla defaultModelPolicy
export function defaultModelPolicy(
  modelId?: string,
  providerType?: GatewayProviderType,
): NonNullable<GatewayModelConfig['policy']> {
  // 🆕 C8.5c: Wykryj thinking-capable model i dostosuj defaults
  const supportsThinking =
  ...
  return {
    ...
    params: {
      defaults: {
        temperature: 0.7,
        // 🆕 C8.5c: Wyższy default dla thinking models
        maxOutputTokens: recommendedMaxTokens,
        // 🆕 C8.5c: thinkingEnabled domyślnie false (opt-in)
        ...(supportsThinking && { thinkingEnabled: false }),
      },
      // 🆕 C8.5c: allowOverrides zawsze zawiera thinking params
      allowOverrides: [
        'temperature',
        'maxOutputTokens',
        'topP',
      ...
      bounds: {
        temperature: { min: 0, max: 2 },
        // 🆕 C8.5c: Wyższy sufit dla thinking models
        maxOutputTokens: {
          min: 1,
          max: supportsThinking ? 16384 : 8192,
        },
...
export class ModelManagerService {
  ...
  async addModelForProvider(
    ...
    while (addMore) {
      ...
      // 🆕 C8.5c: Wykryj thinking-capable model i poinformuj użytkownika
      ...
      config.models[modelAlias] = {
        ...
        capabilities: {
          streaming: true,
          // 🆕 C8.5c: Automatycznie ustaw thinking
          ...(supportsThinking && { thinking: true }),
        },
        // 🆕 C8.5c: Przekaż modelId i type do defaultModelPolicy
    ...
    for (const field of fields) {
      switch (field) {
        case 'modelId': {
          ...
          // 🆕 C8.5c: Po zmianie modelId sprawdź thinking capability
          ...
          if (providerType) {
            ...
            if (supportsThinking && !hadThinking) {
              CliLogger.info(
                `✨ New model supports thinking mode. Update capabilities.thinking manually if needed.`,
              );
            } else if (!supportsThinking && hadThinking) {
              CliLogger.warning(
                `⚠️  New model does NOT support thinking mode. Remove capabilities.thinking manually if needed.`,
              );
            }
          }
          break;
        ...
        case 'policy': {
          // 🆕 C8.5c: Przy edycji policy użyj thinking-aware defaults
          const providerType = config.providers[current.providerInstance]?.type;
          const base =
          ...
          // 🆕 C8.5c: Użyj thinking-aware defaults jako fallback
...
1. **Thinking detection:** Regex patterns w `thinking-capable-models.ts` wykrywają modele wspierające thinking mode:
   - **Anthropic:** Claude Opus 4.x wszystkie, Sonnet 4.5+
   - **Google:** Gemini 3.0+ (ThinkingConfig API od wersji 3.0)
   - **OpenAI:** Poza zakresem (wymaga `/v1/responses` API)

2. **Defaults dla thinking models:**
   - `maxOutputTokens: 8192` (zamiast 1024) — wystarczające dla `thinkingBudget` do ~7680 tokens
   - `bounds.maxOutputTokens.max: 16384` (zamiast 8192) — większa elastyczność
   - `thinkingEnabled: false` — opt-in (wysokie koszty thinking mode)

3. **allowOverrides:** Zawsze zawiera `thinkingEnabled`, `thinkingBudget` — nawet dla non-thinking models (forward compatibility: użytkownik może później włączyć thinking w request).

4. **capabilities.thinking:** Automatycznie ustawione na `true` dla wykrytych modeli (informacyjne, nie wymusza runtime behavior).

5. **UX w CLI:**
   - Po wykryciu thinking-capable model CLI wyświetla `✨ Model ... supports thinking mode`
   - Przy zmianie `modelId` w `model:edit` — ostrzeżenie gdy zmiana wpływa na thinking capability

6. **Backward compatibility:**
   - Istniejące modele bez thinking params — bez zmian (CLI nie nadpisuje ręcznej konfiguracji)
   - Nowe modele z wizarda/`model:add` — automatyczne thinking defaults

**Przykładowe scenariusze:**

| Wizard flow | Model | Generated config | Wynik |
|-------------|-------|------------------|-------|
| `config:init` → Anthropic → `claude-opus-4-8` | Thinking-capable | `maxOutputTokens: 8192`, `thinkingEnabled: false`, `allowOverrides: [..., thinkingEnabled, thinkingBudget]`, `capabilities.thinking: true` | ✅ Ready dla thinking mode |
| `config:init` → Google → `gemini-2.5-flash` | **NIE** thinking-capable (2.5 < 3.0) | `maxOutputTokens: 1024`, `allowOverrides: [..., thinkingEnabled, thinkingBudget]` | ✅ Forward-compatible (może request thinking, ale Gemini 2.5 zignoruje) |
| `config:init` → Google → `gemini-3.0-flash` | Thinking-capable (3.0+) | `maxOutputTokens: 8192`, `thinkingEnabled: false`, `allowOverrides: [..., thinkingEnabled, thinkingBudget]`, `capabilities.thinking: true` | ✅ Ready dla thinking mode |
| `model:add` → Anthropic → `claude-sonnet-4-5-20250929` | Thinking-capable | Jak wyżej | ✅ CLI wyświetla `✨ Model supports thinking mode` |
| `model:edit chat-default` → zmiana `claude-sonnet-4-5` → `gemini-2.5-flash` | Zmiana thinking → non-thinking | CLI wyświetla `⚠️ New model does NOT support thinking mode` | ⚠️ Manualny update `capabilities.thinking` zalecany |

**Interakcja z walidacją C8.5b:**

| Request | Generated config (CLI) | Runtime validation (C8.5b) | Rezultat |
|---------|------------------------|----------------------------|----------|
| `thinkingEnabled: true, thinkingBudget: 2048` | `maxOutputTokens: 8192` (thinking model) | `8192 >= 2560` → ✅ OK | ✅ **PASS** |
| `thinkingEnabled: true, thinkingBudget: 2048` | `maxOutputTokens: 1024` (non-thinking model, ręcznie nie zaktualizowane) | `1024 < 2560` → ❌ 400 | ❌ **FAIL** — jasny komunikat z hintem |
| `thinkingEnabled: true, thinkingBudget: "high"` | `maxOutputTokens: 1024` (dowolny) | String → skip numeric validation → ✅ OK | ✅ **PASS** — string effort nie wymaga wysokiego maxOutputTokens |

**Zalety rozwiązania:**

1. **Automatyzacja:** CLI generuje thinking-ready config bez ręcznej interwencji
2. **Edukacja:** Komunikaty CLI informują użytkownika o thinking capability modeli
3. **Forward compatibility:** `allowOverrides` zawsze zawiera thinking params (nawet dla non-thinking models)
4. **Walidacja runtime:** Cross-validation C8.5b schwyta konflikt budget vs maxOutputTokens (jeśli użytkownik ręcznie obniży maxOutputTokens)
5. **Graceful degradation:** Non-thinking models mają `allowOverrides` thinking params → request z `thinkingEnabled` zostanie zaakceptowany przez gateway (provider zignoruje)

**Commit:** `feat(cli): add thinking mode support in config generation and model management`
...
**Status:** ✅ **WYKONANY 2026-06-15** (kompatybilność API, bez działającej implementacji backendu /v1/responses)
