...
0a. **Fasada (`src/integrations/`) ≠ adapter runtime (`src/providers/`):** szczególnie przy **OpenAI** — fasada `/api/v1/openai/*` to kształt HTTP dla IDE (Cursor); adapter `type: openai` to planowane wywołanie api.openai.com. Warstwy są **ortogonalne**. Skrót: [`dictionary.md`](dictionary.md) (sekcja „Fasada vs provider runtime”), fasada: `integracja-openai-kontrakt.md`, adapter: `provider-openai-runtime.md`.
...
8d. **Extended thinking mode** — wsparcie reasoning models (Anthropic Claude Opus/Sonnet 4.5+, Google Gemini 3.0+); parametry `thinkingEnabled` i `thinkingBudget` w `params`; odpowiedź zawiera opcjonalne pole `thinkingContent`; `capabilities.thinking` w YAML; wysokie koszty 2-10x, domyślnie wyłączone — szczegóły: `dokumentacja_api.md`.
...
22. **Testy** — cztery warstwy: jednostkowe runtime (`npm test`, **68** zestawów / **1114** przypadków, bez `src/cli/`), jednostkowe CLI (`npm run test:cli`, **10** / **42**), E2E HTTP mock (`test/e2e/`, **9** / **91**), integracyjne live SDK+Redis (`test/integration/`, `npm run test:integration`); `npm run test:all` = runtime + E2E — **`testy.md`** (single source of truth dla liczników).
...
- `provider-openai-runtime.md` — **adapter runtime** OpenAI (`type: openai`, `src/providers/`) — plan, mapowanie SDK, status wdrożenia.
