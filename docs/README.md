...
16. **Moduł czatu** — `ChatService` (cache, limity, odpowiedź gateway) + `ChatProviderCallService` (adaptery, metryki, SSE); profile ingress `validateChatIngress` (`native` \| `facade-openai` \| `facade-anthropic`); helpery w `src/chat/helpers/` — `architektura-katalogi-pliki.md`, `data_flow.md`.
