...
22. **Testy** — cztery warstwy: jednostkowe runtime (`npm test`, **86** zestawów / **1305** przypadków, bez `src/cli/`), jednostkowe CLI (`npm run test:cli`, **13** / **53**), E2E HTTP mock (`test/e2e/`, **10** / **105**), integracyjne live SDK+Redis (`test/integration/`, **15** zestawów, `npm run test:integration`); `npm run test:all` = runtime + E2E — **`testy.md`** (single source of truth dla liczników).
