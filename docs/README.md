...
22. **Testy** — cztery warstwy: jednostkowe runtime (`npm test`, **83** zestawów / **1211** przypadków, bez `src/cli/`), jednostkowe CLI (`npm run test:cli`, **13** / **53**), E2E HTTP mock (`test/e2e/`, **10** / **104**), integracyjne live SDK+Redis (`test/integration/`, **14** zestawów, `npm run test:integration`); `npm run test:all` = runtime + E2E — **`testy.md`** (single source of truth dla liczników).
