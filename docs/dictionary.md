...
| **Klucz providera** | Sekret w `.env` do wywołań SDK; w YAML wskazany przez **`apiKeyRef` per `providerInstance`** (np. `ANTHROPIC_PRIMARY_API_KEY`, `GOOGLE_OFFICE_API_KEY`). | Fabryki w `src/providers/factories/`; bootstrap w `ProviderInstancesBootstrap`; nigdy klucz klienta. Legacy nazwy `ANTHROPIC_API_KEY` / `GOOGLE_API_KEY` — opcjonalna synchronizacja z CLI, runtime czyta **`apiKeyRef` z YAML**. |
...
| **Walidacja env (klucze)** | Reguły na zmiennych środowiskowych przy starcie aplikacji. | **Provider:** każda włączona instancja w YAML wymaga niepustego env pod **`apiKeyRef`** (`assertEnabledProviderApiKeysPresent` w `buildEffectiveGatewayConfig`). **Format (opcjonalnie):** gdy ustawione — `ANTHROPIC_API_KEY` musi mieć prefiks `sk-ant-`, `GOOGLE_API_KEY` — `AIza` lub `AQ.` (`env.validation.ts`). Legacy klucze synchronizowane przez CLI — `legacy-provider-env.util.ts`. **`gateway config:validate`** dodatkowo uruchamia `validateEnv()`. |
