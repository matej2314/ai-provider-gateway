...
│   │   │   ├── semantic-cache.service.ts   # embed last-user, KNN, threshold, reuse vector on SET
...
| **`src/cache/`**                         | Response cache for **`POST /api/v1/chat`** — two layers: (1) **exact** (`noop` / `redis` KV, `ResponseCacheService`, reads validated with `CachedChatResponseSchema`); (2) **semantic** (`src/cache/semantic/` — `EmbeddingBackend` + `VectorStore` ports, Ollama + Redis Search KNN adapters, `SemanticCacheService`). **`RedisConnectionService`** — shared Redis infrastructure (exact cache + rate limit + semantic Search); predicate `isRedisRequired()` in `should-include-redis-stack.ts`. Lookup order: exact → semantic → provider. At most one `embed` per JSON request (vector reuse / no retry / first embed if lookup skipped). |
