...
│   │   └── docker-compose.*.yml    # redis (Redis Stack :6380), monitoring, ollama (chat), ollama-embedding (mxbai-embed-large :11435), dev
...
| **`src/cache/`**                         | Response cache for **`POST /api/v1/chat`** — two layers: (1) **exact** (`noop` / `redis` KV, `ResponseCacheService`, reads validated with `CachedChatResponseSchema`); (2) **semantic** (`src/cache/semantic/` — `EmbeddingBackend` + `VectorStore` ports, Ollama + Redis Search KNN adapters, `SemanticCacheService`). **`RedisConnectionService`** — shared Redis infrastructure (exact cache + rate limit + semantic Search); predicate `isRedisRequired()` in `should-include-redis-stack.ts`. Lookup order: exact → semantic → provider. |
