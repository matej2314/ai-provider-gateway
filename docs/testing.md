...
| **Cache (semantic — Faza 2)**    | `src/cache/semantic/**/*.spec.ts` — fake `EmbeddingBackend` (constant vector) + fake `VectorStore`; scenarios: similarity threshold 0.90, skip tooling / `unknown` clientId / no last user message, vector reuse on SET, partition filter by alias+client, fail-open on embedding error, exact key includes `clientId` (isolation regression) |
