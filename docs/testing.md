...
| **Cache (semantic — Faza 2)**    | `src/cache/semantic/**/*.spec.ts` — fake `EmbeddingBackend` (constant vector) + fake `VectorStore`; scenarios: similarity threshold 0.90, skip tooling / `unknown` clientId / no last user message, vector reuse on SET, no `embed` retry when lookup already attempted, first `embed` on store when not attempted, partition filter by alias+client, fail-open on embedding error, exact key includes `clientId` (isolation regression) |
...
**Semantic cache integration (Faza 4):** vector integration tests run against Redis Stack (`redis/redis-stack-server`), **not** the alpine Redis in `test/integration/docker-compose.redis.yml`. A separate Docker stack with Redis Stack is required. The embedding service (Ollama) is **stubbed** with a constant-vector fake — no live Ollama is needed in CI. Exact-cache KV and rate-limit tests continue to use the alpine Redis.
