...
**Fasady IDE** używają tej samej allowlisty kluczy klienta, ale innych nagłówków — Bearer (OpenAI, **wdrożone**) lub `x-api-key` (Anthropic, w przygotowaniu); guard fasady ustawia `req.gatewayKey`, potem `SmartRateLimitGuard` (`readClientGatewayKey`). Klucze providerów w `.env` pozostają wyłącznie w adapterach.
