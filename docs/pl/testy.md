...
| **Cache (semantyczny — Faza 2)** | `src/cache/semantic/**/*.spec.ts` — fałszywy `EmbeddingBackend` (stały wektor) + fałszywy `VectorStore`; scenariusze: próg podobieństwa 0.90, pominięcie tooling / `unknown` clientId / brak ostatniego usera, ponowne użycie wektora przy SET, brak retry `embed` gdy lookup już go wołał, pierwszy `embed` przy zapisie gdy nie był wołany, filtr partycji alias+klient, fail-open przy błędzie embeddingu, klucz exact z `clientId` (regresja izolacji) |
