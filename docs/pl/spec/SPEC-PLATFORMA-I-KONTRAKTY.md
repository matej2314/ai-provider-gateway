...
**Stan implementacji (skrót):** **`openapi.json`** (v0.14.0): czat natywny, SSE, **models**, health (`checks.redis`), fasady OpenAI/Anthropic (`BearerAuth`, `ApiKeyAuth`), `params`, tooling, `cached`/`cachedAt`, retry/fallback/`effectiveModelAlias`, smart rate limit (`src/rate-limit/`), **`RATE_LIMITED`** / **`PROVIDER_RATE_LIMITED`**, **`MODEL_NOT_ALLOWED`**, **`PROVIDER_TIMEOUT`**. Natywny czat i models: **`GlobalExceptionFilter`** (`ErrorEnvelope`); fasady: lokalne filtry + schematy błędów vendora w OpenAPI. **`RequestIdMiddleware`** (nagłówek **`x-request-id`** + pole `requestId` w JSON). **`ChatService`** + **`ChatProviderCallService`**. **`GatewayModelsCatalogService`** (`ModelsModule`). Czat i models: **`@GatewayKeyAndSmartRateLimit()`**. **`ResilientExecutor`** (`src/chat/resilience/`). Konfiguracja runtime: **`AppConfiguration`** + `getAppConfig` (`typed-config.ts`); walidacja env/sekretów: **`ConfigurationValidationService`**. `finishReason`: **`GatewayFinishReason`** (`stop` | `tool_calls` | `length` | `content_filter`).
