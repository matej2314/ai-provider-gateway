...
**Redis:** używa tego samego `RedisConnectionService` i instancji Redis Stack co cache exact i rate limit (port **6380**, obraz `redis/redis-stack-server`). Nazwa indeksu wektorowego to **pełny znormalizowany** `EMBEDDING_MODEL` plus `EMBEDDING_DIM` (trim, lowercase, ciągi niealfanumeryczne → `-`), np. `qwen3-embedding:0.6b` + `1024` → `qwen3-embedding-0-6b-1024`. Warianty tej samej rodziny przy tym samym DIM (np. `qwen3-embedding:4b`) **nie** współdzielą indeksu. Zmiana `EMBEDDING_MODEL` lub `EMBEDDING_DIM` wymaga nowego indeksu. KNN filtruje także TAG-iem `embeddingModel` (skonfigurowany string modelu).
...
| `EMBEDDING_MODEL` | `qwen3-embedding:0.6b` | Model Ollama do embeddingów (`POST /api/embed`). Zmiana modelu (w tym inny tag rozmiaru tej samej rodziny) wymaga nowego indeksu wektorowego — pełna znormalizowana nazwa + DIM, nie krótki slug rodziny. Lżejszy model (np. `nomic-embed-text`) to nowy indeks, nie hot-swap. |
| `EMBEDDING_DIM` | `1024` | Wymiar wektora embeddingu. Musi pasować do modelu (`qwen3-embedding:0.6b` → 1024). Zmiana wartości wymaga nowego indeksu Redis Search (DIM wchodzi w nazwę). |
...
| `SEMANTIC_CACHE_MIN_SIMILARITY` | `0.90` | Minimalne podobieństwo cosinusowe dla trafienia. **Egzekwowane** walidatorem env jako **0–1** włącznie (`@Min(0)` / `@Max(1)`); wartości typu `5` lub `1.01` **psują start**. `gateway config:validate` **ostrzega** (bez fail) gdy wartość jest **&lt; 0.85**. Redis Search przechowuje **dystans** cosinusowy ≈ `1 − podobieństwo`; cutoff ≈ 0.10 przy domyślnym 0.90. |
| `SEMANTIC_CACHE_TTL` | jak `CACHE_TTL` / `3600` | TTL wpisów cache semantycznego (sekundy, ≥ 1). Każdy zapis wektora **zawsze** ustawia TTL (atomowo z `HSET`); brak wiecznych wektorów. |
