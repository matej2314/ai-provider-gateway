...
**Zachowanie:** `ChatService.executeChat` przed wywołaniem providera sprawdza cache (`ResponseCacheService`); przy trafieniu — tylko gdy alias i powiązany provider są **włączone** w YAML (`isCachedChatAllowedForModelAlias` w `src/chat/helpers/cache-policy.ts`) — zwracana jest zapisana odpowiedź z polami **`cached: true`**, **`cachedAt`** (ISO 8601) i **`cacheSource`** (`"exact"` albo `"semantic"`). `cacheSource` nie jest zapisywane w Redis. Odczyt z Redis parsowany jest przez **`parseCachedChatResponse`** (`CachedChatResponseSchema` w `src/cache/schemas/cached-chat-response.schema.ts`); niepoprawny kształt → usunięcie klucza i traktowanie jako MISS. Streaming (`POST /api/v1/chat/stream`) **nie** korzysta z tej warstwy.
