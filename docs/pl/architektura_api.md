...
**Uwaga o idempotencji:** trafienie exact i semantyczne mają `cached: true` / `cachedAt` i są rozróżniane przez **`cacheSource`**. **Nie** są nieograniczonymi substytutami: semantic hit jest ważny tylko w tej samej partycji konfiguracji i przy body jednoturowym. Streaming (`POST /api/v1/chat/stream`) **nie** korzysta z żadnej warstwy cache. JSON fasad OpenAI/Anthropic nie zawiera `cacheSource`; przy hicie fasady ustawiają nagłówek HTTP `X-Gateway-Cache: exact | semantic` (brak przy missie i na streamie).
