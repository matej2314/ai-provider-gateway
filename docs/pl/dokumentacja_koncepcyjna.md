...
- **Endpoint czatu standardowego** `POST /api/v1/chat` — opcjonalnie **cache odpowiedzi** w dwóch warstwach: exact (hash + `clientId`) i semantyczny (embedding + KNN); stream bez cache (`src/cache/`, walidacja odczytu `CachedChatResponseSchema`, env — `konfiguracja.md`).
