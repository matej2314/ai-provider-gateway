...
- **Standard chat endpoint** `POST /api/v1/chat` — optionally **response cache** in two layers: exact (hash + `clientId`) and semantic (embedding + KNN); stream has no cache (`src/cache/`, read validation `CachedChatResponseSchema`, env — `configuration.md`).
