...
**Stan implementacji (skrót):** **`openapi.json`** (v0.13.0): czat natywny, SSE, health (`checks.redis`), fasady OpenAI/Anthropic (`BearerAuth`, `ApiKeyAuth`), `params`, tooling, `cached`/`cachedAt`, retry/fallback/`effectiveModelAlias`, smart rate limit (`src/rate-limit/`), **`RATE_LIMITED`** / **`PROVIDER_RATE_LIMITED`**, **`MODEL_NOT_ALLOWED`**, **`PROVIDER_TIMEOUT`**. Natywny czat: **`GlobalExceptionFilter`** (`ErrorEnvelope`); fasady: lokalne filtry + schematy błędów vendora w OpenAPI. **`RequestIdMiddleware`** (nagłówek **`x-request-id`** + pole `requestId` w JSON). **`ChatService`** + **`ChatProviderCallService`**. Czat: **`@GatewayKeyAndSmartRateLimit()`**. **`ResilientExecutor`**. Konfiguracja runtime: **`AppConfiguration`** + `getAppConfig` (`typed-config.ts`). `finishReason`: **`GatewayFinishReason`** (`stop` | `tool_calls` | `length` | `content_filter`).
