...
**Stan kodu vs te dokumenty:** zestawienie w `docs/dokumentacja_api.md`. **Faza 4 SSE:** zamknięta w kodzie. **Faza 5 (częściowo):** `params` w body (`ChatParamsDto`, `resolveProviderCallOptions`, `MODEL_NOT_ALLOWED`) — **wdrożone**; pozostaje m.in. `config:validate`, response header `x-request-id`. Envelope `ErrorEnvelope`, propagacja `x-request-id` w body, **gateway key (`X-Gateway-Key`)**, limity DTO (`messages` 1–150, `content` max 3000) oraz kody **`MODEL_ALIAS_NOT_FOUND`** / **`STREAMING_NOT_SUPPORTED`** — **wdrożone**; **Faza 6:** observability w toku — `dokumentacja_koncepcyjna.md`. **System prompt** składany po stronie serwera — wdrożony (`konfiguracja.md`, `architektura.md`). **Cache odpowiedzi** dla `POST /api/v1/chat` — wdrożony (`src/cache/`, `konfiguracja.md`); dalszy rozwój Redis: `dokumentacja_koncepcyjna.md`. **Conversation tracking** (`conversationId`, Sentry: pojedyncza wiadomość vs konwersacja od ID w request) — wdrożony (`conversation-tracking.md`, `SPEC-CHAT.md` F-9).
