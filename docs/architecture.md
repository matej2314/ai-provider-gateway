...
| **Cache** (`src/cache`)                                          | Global dynamic module with **two cache layers** for `POST /api/v1/chat`: (1) **Exact cache** — backend registry (`noop` always, `redis` conditionally), `ResponseCacheService`, key from hash of `(modelAlias, clientId, messages, system prompt, effective params)`; entry reads validated with `CachedChatResponseSchema`. (2) **Semantic cache** (`src/cache/semantic/`) — `EmbeddingBackend` port (Ollama adapter) + `VectorStore` port (Redis Search KNN adapter); `SemanticCacheService` embeds the last `role: user` message, queries the nearest-neighbour index, applies similarity threshold. Lookup order: exact → semantic → provider; fail-open on embedding/Search error. **`RedisConnectionService`** (`adapters/redis-cache/`) is shared Redis infrastructure (exact cache + rate limit + semantic cache Search); activation: `isRedisRequiredFromEnv()` in `should-include-redis-stack.ts`. When `SEMANTIC_CACHE_ENABLED=true`, `checks.embeddings` appears in `GET /api/v1/health/ready` (fail-open, does not block `ready`). Env: `configuration.md`. |
