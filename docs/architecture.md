...
flowchart TB
  ...
  subgraph gw [AI Provider Gateway - NestJS]
    ...
    integrations[Integrations Module — IDE facades]
    chat[Chat Module]
...
| **Integrations** (`src/integrations`)                            | OpenAI and Anthropic Messages facades — mapping onto `ChatService` (chat) and `GatewayModelsCatalogService` (models). Anthropic: `anthropic-usage.mapper.ts` (usage JSON ↔ stream), `anthropic-stream.mapper.ts` (thinking in `done` phase). Details: `integrations.md`, `anthropic-messages-integration.md`.                                                                                                                                                                                                                                                                |
...
| **Swagger / OpenAPI** (`src/swagger`)                            | One OpenAPI 3.1 document for **native chat**, **models**, **health**, and **IDE facades** (OpenAI + Anthropic). `@Api*` decorators on all HTTP controllers; `swagger.setup.ts` registers `extraModels` and three `securitySchemes` (`GatewayKeyAuth`, `BearerAuth`, `ApiKeyAuth`). UI: `/api/v1/api-docs`, JSON: `/api/v1/swagger.json`; export: `npm run openapi:export` → `openapi.json`.                                                                                                                                                                   |
...
- **OpenAI** (`openai` 6.x) — `type: openai` in YAML → Responses API; `type: openai-compatible` → Chat Completions (`create-openai-provider.core.ts`); `baseUrl` from env (`baseUrlRef`). Mapping details: `pl/spec/SPEC-PROVIDERS.md`, `provider-openai-runtime.md`.

Broader prompt-layer context: `configuration.md`, `pl/spec/SPEC-KONFIGURACJA.md` (where prompt files are concerned).
