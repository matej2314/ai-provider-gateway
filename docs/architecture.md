...
| **Cache** (`src/cache`)                                          | Global dynamic module with **two cache layers** for `POST /api/v1/chat`: (1) **Exact cache** — backend registry (`noop` always, `redis` conditionally), `ResponseCacheService`, key from hash of `(modelAlias, clientId, messages, system prompt, effective params)`; entry reads validated with `CachedChatResponseSchema`. (2) **Semantic cache** (`src/cache/semantic/`) — `EmbeddingBackend` port (Ollama adapter) + `VectorStore` port (Redis Search KNN adapter); `SemanticCacheService` embeds the last `role: user` message, queries the nearest-neighbour index, applies similarity threshold. Lookup order: exact → semantic → provider; fail-open on embedding/Search error; at most one `embed` per JSON request (reuse lookup vector on store; no retry if `embed` already attempted). **`RedisConnectionService`** (`adapters/redis-cache/`) is shared Redis infrastructure (exact cache + rate limit + semantic cache Search); activation: `isRedisRequiredFromEnv()` in `should-include-redis-stack.ts` — Redis is required when exact cache uses `redis`, smart rate limit is on, **or** `SEMANTIC_CACHE_ENABLED=true` (Search). When `SEMANTIC_CACHE_ENABLED=true`, `checks.embeddings` appears in `GET /api/v1/health/ready` (fail-open, does not block `ready`). Embedding model default: `qwen3-embedding:0.6b` (DIM 1024). Env: `configuration.md`. |
...
| **Health** (`src/health`)                                        | Liveness (`GET /api/v1/health`) and readiness (`GET /api/v1/health/ready` — `checks.config`, `checks.redis`, `checks.cache`, optionally `checks.embeddings`). **`HealthService`**: `evaluateReadiness()`, `publishMetrics()`, warm-up at startup; hook in `PreMetricsScrapeRegistry` (gauges refreshed on `GET /metrics`). **`checks.redis`**: probe of shared Redis infrastructure (`RedisConnectionService.ping()`), only when `isRedisRequiredFromConfig()`; fields `required`, `consumers` (allowed: `cache`, `rate-limit`, `semantic-cache`). **`checks.cache`**: cache feature state; with `redis` backend availability follows from `checks.redis`. **`checks.embeddings`**: embedding service probe, present only when `SEMANTIC_CACHE_ENABLED=true`; fail-open (degraded state does not block `ready`). Configuration validation at **process startup**.                                               |
